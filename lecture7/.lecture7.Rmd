...
stocks %>%
  ...
  pivot_longer(cols = c(`2015`, `2016`), names_to = "year", values_to = "return", names_transform = list(year = as.double))
...
df4 <- df2 %>%
  ...
  # Can do:
  # filter(across(starts_with("wta"), ~ !is.na(.x)))
...
# All together now
who %>%
  pivot_longer(starts_with("new"), names_to = "key", values_to = "cases", values_drop_na = TRUE) %>%
  mutate(key = stringr::str_replace(key, "newrel", "new_rel")) %>%
...
**Exercise 5:** Take the following dataset and tidy it:
