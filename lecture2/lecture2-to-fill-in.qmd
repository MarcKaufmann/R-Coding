...
We will now introduce the `ggplot` library for plotting graphs and illustrate this 
data from the `gapminder` library. In order for this to work, we need to load both
of these. If you get an error when loading them (by calling `library(<package-name>)`), 
then you need to make sure to install them first via `install.package("<package-name>")`.
...
As you can see, the `gapminder` object is a table, with each row containing the 
following information about a given country in a given year:
...
Let us plot the life expectancy against the GDP per capita as a scatter plot:
...
**Class Exercise:** Draw a boxplot of life expectancy for each continent. That is, on 
the x-axis you want to have the continent, on the y-axis life expectancy, and 
instead of using a scatter plot (via `geom_point`) you should use a boxplot. Why 
would it be a bad idea to have country or GDP per capita on the x-axis with boxplot?
...
**Class Exercise:** What other types of plots are there? Try to find several more `geom_` functions.
...
**Class Exercise:** Make sure that your knitted version doesn't include all the output from the `str(...)` commands, it's too tedious. (Hint: see how I did it in other chunks. How could you
...
One thing about the data is that most of it is bunched to the left. If we instead used a logarithmic scale, we should be able to spread the data out better and can fit a
linear line.
...
*Class Exercise:* Make a smooth **orange** line through the scatterplot and give it a size 8.
...
- There are so many points in the scatterplot that it is hard to see much. Let us 
make the points transparent to make high-density areas more visible
- We don't see which points belong to which countries. Let us color points of 
different continents by different colors to provide some indication.
- A good graph should be labeled properly and provide information on data etc.
...
3. In `p2` we repeat several times `color = continent`. Is there a way to avoid it?

**Class Exercise:** Make sure that when you render, you do evaluate the right chunks
in the report (which I switched off to avoid coding errors); to not show messages
that are not helpful in a report; and to not show again and again all the graphs, 
except those that are needed. Thus you may want to include the code, but not the output,
or not even evaluate the code.
...
**Challenge:** How can you plot to `geom`s for two different datasets on the same plot (assuming they share the same x- and y-axis)? For example, suppose that we have data on countries
before 1970 in `gmPre70`, can we then plot a smooth line for the full dataset and
a smooth line for the data prior to 1970 on the same graph?
