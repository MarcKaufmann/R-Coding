...
```{r select-vs-dollar}
#| eval: false
flights$dep_delay # -> vector
select(flights, dep_delay) # -> data frame (tibble)
```
...
p + geom_point(mapping = aes(size = count), alpha = 0.3) + geom_smooth()
...
p2 + geom_point(
  data = flights_delay, 
  mapping = aes(y = avg_arr_delay, size = count),
  alpha = 0.3) +
  geom_smooth()
...
mean(x)
median(x)
x_with_outlier <- c(x, 10000000000)
mean(x_with_outlier)
...
You'll often want to count things, such as the number of flights to each destination.

```{r count-number-of-flights}
library(nycflights13)
library(tidyverse)
...
Or count the number of distinct carriers to each location

```{r count-distinct-carriers}
not_missing |>
  group_by(dest) |>
  summarise(
...
# Count how many airmiles a given airplane did from NYC and arrange:
not_missing |>
