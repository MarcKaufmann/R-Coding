...
- `v`: the original dataset, the one from which you sample the simulated datasets
...
4. Compute the bootstrap standard error for the `median()` rather than `mean()`. Define a function `bootstrap_median` for this.
...
6. Use your new function to compute bootstrap estimators for some linear model coefficients on the vienna dataset. You have to define and name a function that returns the *coefficient* of the right linear model, and pass this function as one of the arguments to `bootstrap_func`.
7. Compare some of the bootstrap estimators to the ones that `summary(lm(...))` spits out. Show the output that gives these estimators.
