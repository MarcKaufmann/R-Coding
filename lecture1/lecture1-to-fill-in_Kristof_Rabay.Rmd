...
- '#' is the commenting sign. It tells R that everything that follows is a comment and R should ignore it. Comments are there to help explain parts of the code that need additional documentation. We will overuse them initially, but reduce this as the course proceeds.
...
It is the process of writing compound words so that the words are separated with an underscore symbol. (Wikipedia)

At first I thought of searching for packages or libraries that are called snake case:
help(snakecase)
No documentation for ‘snakecase’ in specified packages and libraries:
you could try ‘??snakecase’
However I looked it up on Google, there is a package that is used to convert strings to any case and is named SnakeCase (https://cran.r-project.org/web/packages/snakecase/snakecase.pdf)
...
Command results in error, as we have not defined the object that want to know the mean of.
...
class(): a character vector giving the names of the classes from which the object inherits
...
- Only your work until the deadline will be considered. 
