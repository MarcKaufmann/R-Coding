...
df1 <- read_csv("lecture6/test-data1.csv", col_names = FALSE)

df2 <- read_csv("lecture6/test-data2.csv", 
                col_types = cols(
                  col_character(),
                  col_number(), 
                  col_factor()), 
...
# Check tail(df3) to see what goes wrong with that read
tail(read_csv("test-data3.csv", col_typesj = "ccccc"))
tail(read_csv("test-data3.csv", col_types = "nnccf"))
df3 <- read_csv("test-data3.csv", col_types = "nnccf")
tail(df3)
```
...
# Solution: the numbers give the number of the column, if they are given as numbers
# backticks around the name.

table4a %>%
  gather(2, 3, key = "year", value = "cases")

# In order to access a column called `1999` (which is a bad name), you have to put
